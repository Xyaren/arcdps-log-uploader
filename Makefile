...
build_amd64: install_build_dependencies
	...
	GOOS=windows GOARCH=amd64 go build -o ./out/arcdps-log-uploader_amd64.exe -ldflags="-H windowsgui" -i $(moduleName)

.PHONY: build_i386
build_i386: install_build_dependencies
	GOOS=windows GOARCH=386 go generate $(moduleName)
	GOOS=windows GOARCH=386 go build -o ./out/arcdps-log-uploader_i386.exe -ldflags="-H windowsgui" -i $(moduleName)
